...
static int print_device_info(int fd)
{
	...
	if ( debug_flag ) printf("Input device name: \"%s\"\n", name);
...
/* time_start = time_start - new_time; */
/* char *stop_time_str = ctime(&time_start); */
/* stop_time_str[strlen(stop_time_str)-1] = '\0'; */
/* printf("time_start = %s\n", stop_time_str); */
...
static int print_events(int fd)
{
	...
	while (!stop) {
		...
		for (i = 0; i < rd / sizeof(struct input_event); i++) {
			...
			code = ev[i].code;
			value = ev[i].value;
			cn = codename(type, code);
			y_diff = abs(last_y2 - last_y1);
			x_diff = abs(last_x2 - last_x1);
			...
			if (capturing) {
				/* if ( strcmp(cn,"ABS_MT_POSITION_X") != 0 ) {} */

				if (type == EV_ABS && code == ABS_X && value != 0) {
					last_x1 = value;
				}
				if (type == EV_ABS && code == ABS_Y && value != 0) {
					last_y1 = value;
				}
			} else {
			...
			if (type != EV_MSC && (code != MSC_RAW || code != MSC_SCAN)) {
				if ( type == EV_ABS ) {
					if ( capturing && debug_flag ) {
						printf("X=[%d → %d] Δ%d \tY=[%d → %d] Δ%d \t(%s) \n", 
							last_x2,
							last_x1,
							x_diff,
							last_y2, 
			...
			if (type == EV_SYN) {
				if (code == SYN_MT_REPORT) {
					...
					pulse = pulse + 1;
					/* last_ev_timediff = last_ev_time - ev[i].input_event_sec; */
					/* printf("%s\n", codename(type, code)); */ 
				}
			}
			...
			// FIXME: Here's the output printing starts
			/* 	/1* printf("Event: time %ld.%06ld, ", ev[i].input_event_sec, ev[i].input_event_usec); *1/ */
			...
			/* 		else */
			/* 			printf("\n"); */
			/* 			/1* printf("-------------- %s ------------\n", codename(type, code)); *1/ */
			/* 	} else { */
			/* 		// FIXME: Lets only watch for EV_ABS (touchpad movement) */
			/* 		printf("[%d, code %d (%s)]=", */
			/* 			type, */ 
			/* 			code, */ 
			/* 			codename(type, code) */
			...
			/* 			printf("%d\n", value); */
			/* 		} */
			/* 		/1* printf("type %d (%s), code %d (%s), ", *1/ */
			/* 		/1* 	type, *1/ */ 
			/* 		/1* 	typename(type), *1/ */
			/* 		/1* 	code, *1/ */ 
			/* 		/1* 	codename(type, code) *1/ */
			/* 		/1* ); *1/ */

			/* 		/1* if (type == EV_MSC && (code == MSC_RAW || code == MSC_SCAN)) *1/ */
			/* 		/1* 	printf("value %02x\n", ev[i].value); *1/ */
			/* 		/1* else { *1/ */
			/* 		/1* 	printf("value %d\n", ev[i].value); *1/ */
			/* 		/1* } *1/ */
